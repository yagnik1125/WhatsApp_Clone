      ...
      if (await FlutterContacts.requestPermission()) {
        ...
        for (var contact in allContactsInThePhone) {
          for (var firebaseContactData in userCollection.docs) {
            ...
            if (contact.phones[0].number.replaceAll(' ', '') == // aaya haji change thase ghana number ma +91 chhe and ghana ma nathi etle e change karvu padse.
                firebaseContact.phoneNumber) {
              firebaseContacts.add(firebaseContact);
              isContactFound = true;
              break;
            }
            // print("hhhhhhhhhhhh ${contact.phones[0].number}");
          }
          if (!isContactFound) {
            phoneContacts.add(
              UserModel(
